...
function getRoleOccupiedCount(roleId) {
  const placeholders = OCCUPIED_STATUSES.map(() => "?").join(",");
  return db
    .prepare(
      `SELECT COUNT(*) AS c FROM registrations WHERE role_id = ? AND status IN (${placeholders})`,
    )
...
function getRoleStats(activityId) {
  const roles = db
    ...
    SELECT r.*,
      ...
      SUM(CASE WHEN reg.status IN ('待审核', '已选中') THEN 1 ELSE 0 END) AS occupied_count,
